...
onlyfiles = [f for f in listdir('1867') if isfile(join('1867', f))]
...
for file in onlyfiles:
    ...
    for index3,ws in enumerate(wb.worksheets):
        ...
        #Mes com 31 dias
        if ws==wb.worksheets[0] or ws==wb.worksheets[1] or ws==wb.worksheets[3] or ws==wb.worksheets[5] or ws==wb.worksheets[7] or ws==wb.worksheets[8] or ws==wb.worksheets[10] or ws==wb.worksheets[12]:
            if(ws.cell(row=2, column=13).value == None):
                minr=22
                maxc=32
                maxr=52
